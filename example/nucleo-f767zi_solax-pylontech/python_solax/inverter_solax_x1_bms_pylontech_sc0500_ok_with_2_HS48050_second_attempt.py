...
if __name__ == '__main__':
	...
	parser.add_argument("--portinv", default="/dev/ttyACM0", help="""IO bridge interface to the INVerter""")
	parser.add_argument("--baudrateinv", default="115200", help="IOBridge USART speed", type=auto_int)
	parser.add_argument("--portbms", default="/dev/ttyACM1", help="""IO bridge interface to the BMS""")
	parser.add_argument("--baudratebms", default="115200", help="IOBridge USART speed", type=auto_int)
	parser.add_argument("--vmax", default="0", help="Max voltage", type=auto_int)
	parser.add_argument("--vmin", default="0", help="Min voltage", type=auto_int)
	...
	s = serial.Serial(port=args.portinv, baudrate=args.baudrateinv)
	inv = IOBridge(s)

	s2 = serial.Serial(port=args.portbms, baudrate=args.baudratebms)
	bms = IOBridge(s2)

	BMS_RECONNECT_DELAY = 15
	bms_counter = 0
	bms_state = 0
	bms_fault = False
	...
	#start transmission
	bms_reconnect_at = time.time()
	while True:
		if bms_reconnect_at > time.time():
			bms_state = int(time.time()-bms_reconnect_at) + "s"
		try:
			if inv.can_available() > 0:
				cid, cidkind, data = inv.can_rx()
				forward=True
				log.info("inv> " + hex(cid) + " " + cidkind + " " + binascii.hexlify(data).decode("utf8") + " " + str(bms_state))
				# INV_request broadcast?
				if cid == 0x1871 and data[0] == 0x01:
					#to_inv(0x0100A000, 'e', b'', str(bms_state))
					to_inv(0x0100A001, 'e', b'', str(bms_state))
					to_inv(0x1801, 'e', b'\x01\x00\x01\x00\x00\x00\x00\x00', str(bms_state))
					# bms_hannounce
					pass
				if cid == 0x1871 and data[0] == 0x03 and data[1] != 6:
					...
					bms_reconnect_at = time.time() + BMS_RECONNECT_DELAY
					pass
				if time.time() < bms_reconnect_at:
					forward=False
				# detect disconnection
				if cid == 0x1871 and data[0] == 0x02 and data[4] != 1:
					bms_reconnect_at = time.time() + BMS_RECONNECT_DELAY
				if cid == 0x1871 and data[0] != 0x01:
					#when 02 00 01 00 00 00 00 00 is passed to the BMS, it goes to sleep and never wakes up
					forward=False

				if forward:
					to_bms(cid, cidkind, data, str(bms_state))
		...
		try:
			if bms.can_available() > 0:
				...
				log.info("bms> " + hex(cid) + " " + cidkind + " " + binascii.hexlify(data).decode("utf8") + " " + str(bms_state))
				msg=""
				forward=True
				if time.time() < bms_reconnect_at:
					forward = False
				...
				if cid == 0x1877:
					...
					# fixed value
					data = b'\x00\x00\x00\x00\x54\x00'
					if bms_counter == 0:
						data += b'\x00\x02'
					else:
						data += b'\x22' + struct.pack(">B", bms_counter * 16)
					bms_counter=(bms_counter+1)%5
					# last byte must be &0xF, else bms is not recognized

					data = b'\x00\x00\x00\x00\x52\x00\x00\x00'
				# only forward known packet for known interactions :)
				if not cid in (0x1871, 0x1872, 0x1873, 0x1874, 0x1875, 0x1876, 0x1877):
					forward=False
				if forward:
					if bms_state == 2 or True:
						to_inv(cid, cidkind, data, str(bms_state) + "  " + msg)
						if (cid == 0x1877):
							# append additional 1878 message
							data = struct.pack("<HHI", vmax, 0, soh)
							#to_inv(0x1878, 'e', data, str(bms_state))
					# only announce when the BMS is ready
					if (cid == 0x1877):
