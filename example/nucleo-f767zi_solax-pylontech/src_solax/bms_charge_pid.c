...
{
    // ============================================================
    // LAYER 1 — PHYSICAL STATE (hysteresis + safety)
    // ============================================================

    if (soc_full &&
        cell_max_voltage_mV >= ctrl->v_stop_hyst_mV)
    {
        ctrl->charge_allowed = false;
    }
    else if (soc_full &&
             cell_max_voltage_mV < ctrl->v_start_hyst_mV)
    {
    ...
    {
        return ctrl->min_current_offset_dA;
    }

    // ============================================================
    // INVERTER BIAS ESTIMATION (slow observer, NOT control loop)
    // ============================================================

    // estimates: allowed_dA - measured_dA mismatch
    // (this captures hidden inverter behavior / PV collapse effects)

    int16_t last_allowed_dA = ctrl->last_allowed_dA;

    int16_t estimated_bias =
        last_allowed_dA - measured_current_dA;

    ctrl->inverter_bias_dA =
        (ctrl->inverter_bias_dA * 31 + estimated_bias) / 32;
    ...
    // energy error (slow loop)
    int16_t energy_error_dA =
        target_current_dA - avg_current_dA;

    // asymmetric response (VERY IMPORTANT for PV stability)
    int16_t energy_correction_dA;

    if (energy_error_dA > 0)
    {
        // deficit → allow stronger correction
        energy_correction_dA = (energy_error_dA * ctrl->ki_up_x100) / 100;
    }
    else
    {
        // recovery → damp correction
        energy_correction_dA = (energy_error_dA * ctrl->ki_down_x100) / 100;
    }

    // clamp energy correction
    ...
    // ============================================================
    // LAYER 3 — FAST DYNAMICS (PID on instantaneous response)
    // ============================================================

    int16_t error_dA = target_current_dA - measured_current_dA;

    int32_t p =
        ((int32_t)ctrl->kp_x100 * error_dA) / 100;

    int32_t i =
        ctrl->integral_x100 / 100;

    int32_t d =
        ((int32_t)ctrl->kd_x100 *
    ...
    // rate limiting
    if (dynamic_dA > ctrl->max_step_up_dA)
        dynamic_dA = ctrl->max_step_up_dA;

    if (dynamic_dA < -ctrl->max_step_down_dA)
        dynamic_dA = -ctrl->max_step_down_dA;

    // ============================================================
    // COMBINE LAYERS (NO measured_current injection anymore)
    // ============================================================
    ...
    // ============================================================
    // HARD PHYSICAL CLAMPS
    // ============================================================

    if (allowed_dA > target_current_dA)
    {
        allowed_dA = target_current_dA;
        ctrl->integral_x100 -= ctrl->integral_x100 / 10;
    }

    // inverter bug, must always at least be ready to sink current
    if (allowed_dA < ctrl->min_current_offset_dA)
    {
        allowed_dA = ctrl->min_current_offset_dA;
    }

    // store for bias estimator
...
)
{
    ...
    // ============================================================
    // LAYER 2 — ENERGY LOOP (slow average regulation)
    // ============================================================
    ...
    // ============================================================
    // LAYER 2.5 — BIAS ESTIMATION (adaptive inverter compensation)
    // ============================================================

    int16_t bias_error_dA =
        target_current_dA - measured_current_dA;

    // slow adaptation
    ctrl->bias_correction_dA += bias_error_dA / 32;

    // clamp bias to safe bounds
    if (ctrl->bias_correction_dA > 100)
        ctrl->bias_correction_dA = 100;

    if (ctrl->bias_correction_dA < -100)
        ctrl->bias_correction_dA = -100;

    // ============================================================
    // LAYER 3 — FAST PID (instantaneous dynamics)
    // ============================================================

    int16_t effective_target_dA =
        target_current_dA
    ...
    // ============================================================
    // DESIRED OUTPUT (unconstrained)
    // ============================================================

    // int32_t desired_allowed_dA =
    //     (int32_t)measured_current_dA
    //     + dynamic_dA
    //     + energy_correction_dA;
    int32_t desired_allowed_dA =
    ...
    // ============================================================
    // RATE LIMITING (delta-based)
    // ============================================================

    int32_t delta_dA =
        desired_allowed_dA - ctrl->last_allowed_dA;

    if (delta_dA > ctrl->max_step_up_dA)
        delta_dA = ctrl->max_step_up_dA;

    if (delta_dA < -ctrl->max_step_down_dA)
        delta_dA = -ctrl->max_step_down_dA;

    int32_t allowed_dA =
        ctrl->last_allowed_dA + delta_dA;

    // ============================================================
    // ENERGY SAFETY (acts on REAL current, not allowed)
    // ============================================================

    if (avg_current_dA > target_current_dA + 5)
    {
        // gently reduce integral to avoid sustained overshoot
        ctrl->integral_x100 -= ctrl->integral_x100 / 8;
    }

    // ============================================================
    // MIN CURRENT OFFSET (inverter quirk protection)
    // ============================================================
    ...
    if (allowed_dA > 2*ctrl->bias_correction_dA + target_current_dA)
        allowed_dA = 2*ctrl->bias_correction_dA + target_current_dA;
...
)
{
    ...
    // ============================================================
    // LAYER 2.5 — BIAS LEARNER (inverter offset compensation)
    // ============================================================
    ...
    // ============================================================
    // LAYER 3 — FAST PID (TRACK TARGET ONLY)
    // ============================================================
    ...
    if (allowed_dA > effective_target_dA + ctrl->inverter_offset_dA + ctrl->bias_correction_dA)
        allowed_dA = effective_target_dA + ctrl->inverter_offset_dA + ctrl->bias_correction_dA;
    ...
    printf(
      "P=%d I=%d D=%d dyn=%d corr=%d last=%d meas=%d allowed_pre=%d allowed_pre_rl=%d allowed_pre_zal=%d allowed_pre_clamp=%d allowed_post=%d bias=%d\n",
      p, i, d,
      dynamic_dA,
      energy_correction_dA,
...
)
{
    ...
    // // ============================================================
    // // HARD SAFETY: force correction when measured > target
    // // ============================================================
    ...
    //     // force stronger negative action
    //     allowed_dA -= excess * 2;   // aggressive braking

    //     // also kill integrator pushing the wrong way
