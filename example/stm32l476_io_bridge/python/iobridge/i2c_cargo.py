...
if __name__ == '__main__':
	...
	# TODO set MTU with the device
	mtu = args.mtu

	for line in sys.stdin:
		line = line.rstrip("\n").rstrip("\r")
		chunks = fragment(binascii.unhexlify(line), mtu)
		for c in chunks:
			iob.i2c_write(args.addr, c)
			iob.i2c_wait_interrupt()
			rarray = iob.i2c_read(args.addr, 3)
			nb = cargo_size(rarray)
			while nb:
				si = min(nb, mtu)
				rarray += iob.i2c_read(args.addr, si)
				nb -= si
